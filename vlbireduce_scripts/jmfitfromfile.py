...
usage =  "usage: jmfitfromfile.py <image fits file> <output stats file (only name of file)> " +\
            "[pixwindow | pixwindowx,pixwindowy | blc,blc,trc,trc | blc,blc,blc,trc,trc,trc]]\n"
...
jmfitfile = os.path.dirname(imagefile) + '/' + sys.argv[2]
pixwindowx = 50
pixwindowy = 50
blc=[None]
trc=[None]
...
# source block 
while jmfitmessage[msgindex].find('Source') == -1 and \
          msgindex < len(jmfitmessage):
    msgindex = msgindex + 1
source = jmfitmessage[msgindex].split(' ')[2]
...
targetstatout.write(f"Source {source}: MJD " + str(mjd) + ":\n")
